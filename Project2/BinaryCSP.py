...
class FourQueenConstraint(BinaryConstraint):
    def isSatisfied(self, value1, value2):
        ...
        sameDiag = True
        row1 = int(str(value1))
        col1 = int(str(self.var1))
        row2 = int(str(value2))
        col2 = int(str(self.var2))

        if row1 is not row2:
            sameRow = False

        if abs(row1 - row2) is not abs(col1 - col2):
            sameDiag = False

        return not sameRow and not sameDiag

    def __repr__(self):
        return 'FourQueenConstraint (%s, %s)' % (str(self.var1), str(self.var2))
...
def consistent(assignment, csp, var, value):
    """Question 1"""
    assignedVals = assignment.assignedValues

    for constraint in csp.binaryConstraints:
        if constraint.affects(var):
            tempVar = constraint.otherVariable(var)
            tempVal = assignedVals[tempVar]
            if not constraint.isSatisfied(value, tempVal):
                return False
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
    ...
    vals = orderValuesMethod(assignment, csp, var)

    for val in vals:
        if consistent(assignment, csp, var, val):
            assignment.assignedValues[var] = val
            ret = recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod)
...
def minimumRemainingValuesHeuristic(assignment, csp):
    ...
    """Question 2"""
    unassignedVars = []
    assignmentDict = assignment.assignedValues
    constraints = csp.binaryConstraints

    for var in assignmentDict.keys():
        if assignmentDict[var] is None:
            unassignedVars.append(var)

    nextVar = unassignedVars[0]
    minVals = len(domains[nextVar])
    minVars = 0

    for val in domains[nextVar]:
        if consistent(assignment, csp, nextVar, val):
            minVars += 1

    for var in unassignedVars:
        #MRV
        if len(domains[var]) < minVals:
            minVals = len(domains[var])
            nextVar = var
        #Degree heuristic
        elif len(domains[var]) == minVals:
            varCount = 0
            ...
            if varCount < minVars:
                nextVar = var
                minVars = varCount
            elif varCount == minVars:
                currDegree = 0
                nextDegree = 0
                for constraint in constraints:
                    if constraint.affects(var):
                        otherVar = constraint.otherVariable(var)
                        if otherVar in unassignedVars:
                            currDegree += 1
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    for constraint in csp.binaryConstraints:
        if constraint.affects(var):
            constrainedVars.append(constraint.otherVariable(var))

    for val in values:
        count = 0
        for v in constrainedVars:
            if val in list(assignment.varDomains[v]):
                count += 1

        constrainedVals[val] = count

    constrainedVals = sorted(constrainedVals, key=constrainedVals.get)
    retList = []
    for pair in constrainedVals:
        retList.append(pair[0])
...
def forwardChecking(assignment, csp, var, value):
    ...
    for constraint in csp.binaryConstraints:
        if constraint.affects(var):
            otherVar = constraint.otherVariable(var)
            if assignmentDict[otherVar] is None:
                constrainedVars.append(otherVar)

    for cvar in constrainedVars:
        if value in domains[cvar]:
            if len(domains[cvar]) == 1:
                for visited in visitedVars:
                    domains[visited].add(value)
                return None
            else:
                visitedVars.append(cvar)
                inferences.add((cvar, value))
                domains[cvar].remove(value)
    ...
    for val in vals:
        if consistent(assignment, csp, var, val):
            ...
            if inferences is not None:
                assignment.assignedValues[var] = val
                ret = recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod)
                if ret is not None:
                    return ret
                else:
...
def maintainArcConsistency(assignment, csp, var, value):
    ...
    while len(queue) > 0:
        ...
        revisions = revise(assignment, csp, params[0], params[1], params[2])
        ...
        if revisions is None:
            ...
            for constraint in csp.binaryConstraints:
                currVar = params[1]
                if constraint.affects(currVar):
                    otherVar = constraint.otherVariable(currVar)
                    if assignmentDict[otherVar] is None:
                        queue.append((currVar, otherVar, constraint))
