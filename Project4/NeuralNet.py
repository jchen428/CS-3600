    ...
    def feedForward(self, inActs):
        ...
        for i in range(0, self.numLayers):
            ...
            for perceptron in self.layers[i]:
                next.append(perceptron.sigmoidActivation(input))
            ret.append(next)
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    while True:
        ...
        trainError = backProp[0]
        weightMod = backProp[1]
        ...
        if weightMod < weightChangeThreshold or iteration > maxItr:
            break
    ...
    for example in examplesTest:
        ...
        rounded = []
        for val in result[len(result) - 1]:
            rounded.append(round(val))

        if example[1] == rounded:
            testCorrect += 1
        else:
            testError += 1
