...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostDist in livingGhostPositionDistributions:
            maxProbs.append(ghostDist.argMax())

        shortestDist = 99999
        closestGhost = None
        for pos in maxProbs:
            distToGhost = self.distancer.getDistance(pacmanPosition, pos)
        ...
        for a in legal:
            ...
            distToGhost = self.distancer.getDistance(newPos, closestGhost)
            if distToGhost < currDist:
                currDist = distToGhost
                action = a
